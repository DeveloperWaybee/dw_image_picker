...
public class DWImagePickerPlugin: NSObject, FlutterPlugin, TLPhotosPickerViewControllerDelegate, CropViewControllerDelegate, UIImagePickerControllerDelegate & UINavigationControllerDelegate {
    ...
    private func openCamera() {
        if UIImagePickerController.isSourceTypeAvailable(UIImagePickerController.SourceType.camera) {
            DispatchQueue.main.async {
                DWImagePickerUtils.checkCameraPermission { granted in
                    if granted {
                        DispatchQueue.main.async {
    ...
    public func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey: Any]) {
        ...
        if isVideo {
            if let videoURL = info[.mediaURL] as? URL {
                ...
                let videoSize = DWImagePickerUtils.getVideoSize(asset: asset)
                let mimeType = DWImagePickerUtils.getMimeType(url: videoURL)
                var media = [
                    "path": pathStr,
                    "id": pathStr,
                    ...
                    "size": DWImagePickerUtils.getFileSize(at: videoURL.path),
                    "type": "video"
                ] as [String : Any]
                ...
                if isGenerateThumbnail {
                    ...
                    media["thumbnail"] = DWImagePickerUtils.generateVideoThumbnail(from: videoURL ,quality: compressQuality, format: compressFormat)
                }
                result!(media)
            }
            ...
            if let image = info[.originalImage] as? UIImage {
                if isCropEnabled {
                    ...
                    let imageData = DWImagePickerUtils.copyImage(image, quality: compressQuality, format: compressFormat, targetSize: targetSize)
                    result!(imageData)
                    picker.dismiss(animated: true, completion: nil)
                }
            } else {
        ...
        if (configure.mediaType == .image && isCropEnabled) {
            ...
            for asset in withTLPHAssets {
                ...
                if (isConvertHeic && isHeicPhoto && !isLivePhoto) || isCompressImage, let uiImage = asset.fullResolutionImage {
                    if let imageInfo = DWImagePickerUtils.copyImage(uiImage, quality: compressQuality, format: compressFormat, targetSize: targetSize, id: asset.phAsset?.localIdentifier) {
    ...
    private func buildResponse(path: URL, withType type: String, withAsset asset: TLPHAsset ) -> [String : Any] {
        ...
        if phAsset?.mediaType == .video {
            ...
            if isGenerateThumbnail {
                let compressQuality = arguments?["thumbnailCompressQuality"] as? Double
                let compressFormat = arguments?["thumbnailCompressFormat"] as? String
                media["thumbnail"] = DWImagePickerUtils.generateVideoThumbnail(from: path ,quality: compressQuality, format: compressFormat)
            }
    ...
    public func cropViewController(_ cropViewController: CropViewController, didCropToImage image: UIImage, withRect cropRect: CGRect, angle: Int) {
        ...
        let croppedImage = DWImagePickerUtils.copyImage(image, quality: compressQuality, format: compressFormat, targetSize: targetSize)
        DispatchQueue.main.async {
